...
if (Meteor.isClient) {
  ...
  var OpenButton = React.createClass({
    ...
    _openOverlay: function(event) {
      React.render(
        <Overlay>
          <h3>&lt;Leaderboard&gt; in overlay, press ESC</h3>
          <Leaderboard/>
        </Overlay>
    ...
    render: function() {
      return (
        <button onClick={this._openOverlay}>Open in Overlay</button>
      );
    }
  });
  ...
  Meteor.startup(function (argument) {
    React.render(
      <div>
        <OpenButton/>
        <h3>&lt;Leaderboard&gt; in body</h3>
        <Leaderboard/>
      </div>
      , document.getElementById('leaderboard_placeholder'));
  });
